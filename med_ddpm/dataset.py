...
class NiftiPairImageGenerator(Dataset):
    ...
    def resize_img_4d(self, input_img):
        d, h, w, c = input_img.shape  # axial 기준이면 이렇게
        result_img = np.zeros((self.depth_size, self.input_size, self.input_size, c))
        if d != self.depth_size or h != self.input_size or w != self.input_size:
            for ch in range(c):
                buff = input_img[..., ch]  # shape: (D, H, W)
